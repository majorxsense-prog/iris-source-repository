...
async function chapters(title) {
  ...
  const results = [];
  const seen = {};
  const chapterPattern = /<a[^>]+href=["']([^"']*\/(?:webtoon|manga)\/[^"']+\/chapter-[^"']+\/?)["'][^>]*>([\s\S]*?)<\/a>/gi;
  let match;

  while ((match = chapterPattern.exec(html)) !== null) {
    const url = absoluteURL(match[1]);
    const id = pathFromURL(url);
    if (seen[id]) {
      continue;
    }
    seen[id] = true;
...
async function chapterDetails(title, chapter) {
  const html = await htmlGet(chapterPath(title, chapter));
...
function parseTitleList(html, limit) {
  ...
  while ((match = anchorPattern.exec(html)) !== null && items.length < (limit || 20)) {
    const url = absoluteURL(match[1]);
    const slug = match[2];
    if (!slug || slug.startsWith("chapter-") || seen[slug]) {
      continue;
    }
    seen[slug] = true;
    const block = nearbyBlock(html, match.index);
    const title = htmlText(match[3]) || htmlText(firstMatch(block, /alt=["']([^"']+)["']/i)) || slug.replace(/-/g, " ");
...
function parseReaderImages(html) {
  const content = firstMatch(html, /<div[^>]*class=["'][^"']*(?:reading-content|entry-content|chapter-content)[^"']*["'][^>]*>([\s\S]*?)(?:<\/div>\s*<\/div>|<\/article>|<div[^>]*class=["'][^"']*nav-links)/i) || html;
  const images = [];
  const seen = {};
  const imagePattern = /<img[^>]+(?:data-src|data-lazy-src|src)=["']([^"']+)["'][^>]*>/gi;
  let match;

  while ((match = imagePattern.exec(content)) !== null) {
    const url = decodeHTML(match[1]).trim();
    if (!url || seen[url] || url.includes("logo") || url.startsWith("data:")) {
      continue;
    }
    seen[url] = true;
...
function nearbyBlock(html, index) {
...
function firstImage(html) {
  const value = firstMatch(html, /<img[^>]+(?:data-src|data-lazy-src|src)=["']([^"']+)["'][^>]*>/i);
  return value ? absoluteURL(decodeHTML(value)) : null;
}
